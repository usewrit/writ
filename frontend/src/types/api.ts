...
export interface WorkflowStep {
  id?: string;
  type: 'navigate' | 'navigated_to' | 'click' | 'fill' | 'type' | 'select' | 'wait' | 'screenshot' | 'extract' | 'evaluate' | 'api_call' | 'login_post' | 'wait_for_tab' | 'open_tab' | 'tab_closed' | 'assert' | 'ai_fill' | 'ai_continue' | 'ai_navigate' | 'codegen' | 'press' | 'scroll' | 'scroll_into_view' | 'hover' | 'check' | 'uncheck' | 'end_point' | 'captcha' | 'wait_for_change' | 'upload' | 'wait_for_download' | 'advanced_script';
  selector?: string;
  value?: string;
  url?: string;
  timeout_ms?: number;
